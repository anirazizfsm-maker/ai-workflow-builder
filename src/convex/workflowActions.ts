...
export const generateWorkflowJSON = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (prompt.toLowerCase().includes("report") || prompt.toLowerCase().includes("daily")) {
      ...
      return {
        ...
        description: `Generated workflow: ${prompt}`,
      };
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      throw new Error(`Failed to generate workflow JSON: ${message}`);
    }
  },
});