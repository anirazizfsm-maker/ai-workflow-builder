...
const GooeyNav = ({
  ...
  const createParticle = (
    ...
    return {
      ...
      end: getXY(d[1] + (noise(7) as number), particleCount - i, particleCount),
      time: t,
      scale: 1 + (noise(0.2) as number),
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      const t = animationTime * 2 + (noise(timeVariance * 2) as number);
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
    e.preventDefault(); // keep SPA feel; parent page can link via navigate() separately
    ...
    if (filterRef.current) {
      ...
      particles.forEach((p) => (filterRef.current as HTMLElement).removeChild(p));
    }
    ...
    if (filterRef.current) {
      makeParticles(filterRef.current as HTMLElement);
    }

    // After animation trigger, navigate via changing location (consumer can also intercept)
    const href = items[index]?.href;
    if (href) window.history.pushState({}, "", href);
    // fire a popstate so SPA routers can respond if listening
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLAnchorElement>, index: number) => {
    if (e.key === "Enter" || e.key === " ") {
      ...
      const liEl = (e.currentTarget as HTMLAnchorElement).parentElement as HTMLLIElement;
      if (liEl) {
        // simulate click without following href
        setActiveIndex(index);
        updateEffectPosition(liEl);
        if (filterRef.current) {
          const particles = filterRef.current.querySelectorAll(".particle");
